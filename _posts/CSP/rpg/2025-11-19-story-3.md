...
<div class="container">
    ...
    <div class="generator-card">
        <h2 class="generator-title">🗺️ World Map Quest Tracker</h2>
    ...
    <div class="generator-card">
        <h2 class="generator-title">🎨 Character Builder</h2>
        ...
        <div id="character-sheet-result" class="result-box" style="margin-top: 30px;">
            ...
            <button onclick="downloadCharacterSheet()" class="generator-btn" style="margin-top: 20px; padding: 12px 30px; font-size: 1em;">
                <span style="position: relative; z-index: 1;">📥 Download Character Sheet</span>
            </button>
        </div>
    </div>
...
async function loadQuestLog() {
    try {
        const response = await fetch('http://localhost:8587/api/rpg/quests');
        if (response.ok) {
            const result = await response.json();
